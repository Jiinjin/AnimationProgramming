...
bool ImGui::BeginDragDropSource(ImGuiDragDropFlags flags)
{
    ...
    bool source_drag_active2 = false;
    ImGuiID source_id = 0;
    ImGuiID source_parent_id = 0;
    ImGuiMouseButton mouse_button = ImGuiMouseButton_Left;
    ImGuiMouseButton mouse_button2 = ImGuiMouseButton_Right;
    if (!(flags & ImGuiDragDropFlags_SourceExtern))
    {
        source_id = window->DC.LastItemId;
        if (source_id != 0 && g.ActiveId != source_id) // Early out for most common case
            return false;
        if (g.IO.MouseDown[mouse_button] == false && g.IO.MouseDown[mouse_button2] == false)
            return false;
        if (source_id == 0)
        {
            ...
            else if (is_hovered && g.IO.MouseClicked[mouse_button2])
            {
                SetActiveID(source_id, window);
                FocusWindow(window);
            }
        ...
        source_drag_active2 = IsMouseDragging(mouse_button2);
    ...
    if (source_drag_active || source_drag_active2)
    {
        if (!g.DragDropActive)
        {
            ...
            {
                g.DragDropMouseButton = mouse_button2;
